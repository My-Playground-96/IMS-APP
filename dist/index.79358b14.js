...
function loadAllCourses() {
    fetch(`http://localhost:8080/courses`).then((res)=>{
        if (res.ok) res.json().then((coursesList)=>{
            coursesList.forEach((course)=>{
                CourseID = course.courseId + 1;
                addCourseToTable(course);
            });
        });
        else alert("Failed to load the courses list!");
    }).catch((err)=>{
...
btnSave.addEventListener("click", (e)=>{
    ...
    // Validate the inputs
    if (validateForm()) {
        if (isUpdateMode) fetch(`http://localhost:8080/courses/${id}`, {
            method: "PATCH",
            headers: {
        ...
        else fetch("http://localhost:8080/courses", {
            method: "POST",
            headers: {
                "content-Type": "application/json"
            },
...
function filterTableRows(query) {
    ...
    rows.forEach((row)=>{
        ...
        if (userId.includes(query) || userName.includes(query) || userAddress.includes(query)) row.style.display = "";
        else row.style.display = "none";
    });
}

},{}]},["7OWwZ","eM5YZ"], "eM5YZ", "parcelRequire8d29")

//# sourceMappingURL=index.79358b14.js.map
